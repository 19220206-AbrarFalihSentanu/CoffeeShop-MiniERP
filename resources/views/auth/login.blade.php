<x-guest-layout>
    ...
    <h4 class="text-xl font-bold text-center text-[#8B5A2B] mb-2">{{ __('auth.login') }}</h4>
    <p class="text-center text-gray-500 text-sm mb-6">Masuk untuk mengelola akun Anda</p>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="flex items-center mt-4">
            <input id="remember_me" type="checkbox"
                class="w-4 h-4 rounded border-gray-300 text-[#8B5A2B] shadow-sm focus:ring-[#8B5A2B] focus:ring-offset-0"
                name="remember">
        ...
        <div class="flex items-center justify-center mt-4 text-sm">
            @if (Route::has('password.request'))
                <a class="text-[#8B5A2B] hover:text-[#6F4E37] hover:underline" href="{{ route('password.request') }}">
